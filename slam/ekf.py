...
class EKF:
    ...
    def __init__(self, robot):
        ...
        self.R_Factor = 0 #0.05**2
        self.Q_Factor = 0.004 # Original 0.3
    ...
    def update(self, measurements):
        ...
        for i in range(len(measurements)):
            R[2*i:2*i+2,2*i:2*i+2] = measurements[i].covariance + self.R_Factor*np.eye(2) #Edit later R_factor is new addition
    ...
    def predict_covariance(self, raw_drive_meas):
        n = self.number_landmarks()*2 + 3
        Q = np.zeros((n,n))
        Q[0:3,0:3] = self.robot.covariance_drive(raw_drive_meas)+ self.Q_Factor*np.eye(3) #Edit 0.004 later if results dont Improve
        return Q
