...
def parse_robot_pose(fname : str) -> dict:
    with open(fname, 'r') as f:
        Robot_dict = ast.literal_eval(f.read())
        pose_dict = {}
        for (i) in range(3):
            pose_dict[i] = Robot_dict["RobotPose"][i]
    return pose_dict
...
def custom_apply_transform(theta, x, points):
    ...
    points_transformed =  np.linalg.inv(R) @ points + x
    return points_transformed
...
if __name__ == '__main__':
    ...
    poseDict = parse_robot_pose(args.estimate)
    ...
    theta =  poseDict[2][0]
    x = np.array([poseDict[0][0], poseDict[1][0]]).reshape(2,1)
    us_vec_aligned = custom_apply_transform(theta, x, us_vec)
    ...
    print()
    print("The following parameters have been transformed.")
    print("Rotation Angle: {}".format(theta))
    print("Translation Vector: ({}, {})".format(x[0,0], x[1,0]))
    ...
    print('-----------------------------------------------------------------')
    for i in range(len(taglist)):
        print('%3d %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f\n' % (taglist[i], gt_vec[0][i], us_vec[0][i], diff2[0][i], gt_vec[1][i], us_vec[1][i], diff2[1][i]))
    ...
    ax.scatter(us_vec[0,:], us_vec[1,:], marker='o', color='C2', s=100)
    ...
    for i in range(len(taglist)):
        ...
        ax.text(us_vec[0,i]+0.05, us_vec[1,i]+0.05, taglist[i], color='C2', size=12)
    ...
    plt.legend(['Real','PredAligned','PredNonAligned'])
    plt.grid()
    plt.show()